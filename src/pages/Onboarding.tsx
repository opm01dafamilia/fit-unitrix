...
terms_accepted: false as boolean
  ...
  { title: "Experiência e Local", desc: "Seu nível e onde treina", icon: Dumbbell, fields: ["experience_level", "training_location"] },
  {
    ...
    desc: "Vamos ajustar seu treino ideal",
    icon: Dumbbell,
    fields: ["training_days", "intensity_preference", "workout_time", "has_pain", "pain_notes"]
  },
];
  ...
  const handleFinish = async () => {
    ...
    else { await refreshProfile(); toast.success("Perfil configurado!"); navigate("/dashboard"); }
  };
  ...
  <Select
    ...
    onValueChange={(v) => setData({ ...data, training_days: v })}
  >
...
<div>
  ...
  <Select
    value={data.intensity_preference}
    onValueChange={(v) => setData({ ...data, intensity_preference: v })}
  >
    <SelectTrigger className="bg-secondary/50 border-border/50">
      <SelectValue placeholder="Selecione" />
    </SelectTrigger>
...
<div>
  ...
  <Select
    ...
    onValueChange={(v) => setData({ ...data, workout_time: v })}
  >
    <SelectTrigger className="bg-secondary/50 border-border/50">
      <SelectValue placeholder="Selecione" />
    </SelectTrigger>
...
<div>
  ...
  <Select
    value={data.has_pain}
    onValueChange={(v) => setData({ ...data, has_pain: v })}
  >
    <SelectTrigger className="bg-secondary/50 border-border/50">
      <SelectValue placeholder="Selecione" />
    </SelectTrigger>
  ...
  <Input
    placeholder="Ex: dor no joelho, ombro..."
    value={data.pain_notes}
    onChange={(e) => setData({ ...data, pain_notes: e.target.value })}
    className="bg-secondary/50 border-border/50"
  />
</div>

<div className="mt-4 p-3 border border-border/50 rounded-lg bg-secondary/30">
  <p className="text-[11px] text-muted-foreground leading-relaxed">
    Este aplicativo gera treinos utilizando inteligência artificial com base nas informações fornecidas.
    As recomendações servem apenas como orientação e não substituem a avaliação de um profissional qualificado.
    Em caso de dores, limitações ou condições específicas, procure um médico ou instrutor.
  </p>
